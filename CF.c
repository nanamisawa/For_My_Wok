...
{
    int i=-2,j=1,AtomNo1=0,AtomNo2=0,AtomNo3=0,AtomNo4=0;
    char str[N];
    FILE *fi;
...
void calc_distance()
{
    ...
    double x,y,z,powx2,powy2,powz2,dist; //for non-pbc
    for (j = 1; j < (StepNum+1); j++){
        for (i = 1; i < (CandNum+1); i++){
            ...
            //From here non-pbc
            x = CrdList[step][atomNoA*3-2]-CrdList[step][atomNoB*3-2];
            y = CrdList[step][atomNoA*3-1]-CrdList[step][atomNoB*3-1];
            z = CrdList[step][atomNoA*3]-CrdList[step][atomNoB*3];
            powx2 = pow(x,2.0);
            powy2 = pow(y,2.0);
            powz2 = pow(z,2.0);
            dist = sqrt(powx2+powy2+powz2);
